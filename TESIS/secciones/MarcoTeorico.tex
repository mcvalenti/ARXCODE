...
\begin{itemize}
 ...
 \item Restringir el modelo gravitacional de Brouwer s\'olo a los t\'erminos de per\'iodo largo o corto, que no contengan a la excentricidad como factor.
\end{itemize}
...
Tal como lo menciona Lei-Chen \citep{leichen}, muchos estudios analizan la precisi\'on de las posiciones de los TLE compar\'andolas con efem\'erides precisas ({\color{red}{hacer/Ver tabla}}). Pero como se menciona en el p\'arrafo anterior, esto no es pr\'actico cuando se trata de desechos, ya que no se tienen efem\'erides precisas de los mismos. En estos casos, los m\'etodos de comparaci\'on de pares resultan una opci\'on v\'alida. El concepto b\'asico consiste en comparar posiciones orbitales en la \'epoca del TLE contra posiciones predichas usando TLE anteriores y el modelo de propagaci\'on SGP4. Los residuos se analizan estad\'isticamente para obtener informaci\'on de los errores. ({\color{red}{resumen de trabajos sobre esto}})

\begin{table}
 \centering
  \begin{tabular}{lc}
  \hline \hline
  Autore/s & Descripci\'on del trabajo \\
  \hline \hline
  Flohrer  & look-up\\
  Wang \citep{wang2009propagation}& B\\
  \end{tabular}
\end{table}
